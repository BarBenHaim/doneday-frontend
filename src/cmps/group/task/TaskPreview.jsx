  ...
  const renderCell = (config, task, members, labels, key) => {
    ...
    return (
      <TableCell
        key={key}
        className={className || ''}
        style={{ width, flexGrow: key === 'addColumn' ? 1 : 0 }}
      >
        {render
  ...
  return (
    ...
    >
      ...
      <TableRow className="task-preview-row" highlighted={highlighted}>
        {cmpsOrder.map((key) =>
          renderCell(
            taskAttributesConfig[key.match(/^\D+/)[0]],
            task,
            members,
            labels,
            key
        ...
        <TableCell
          key="addColumn"
          className="table-cell add-column-cell"
          style={{ width: '100%' }}
        >
          {isPlaceholder && (
            <input type="text" onChange={handlePlaceholderInput} />
          )}
        </TableCell>
      </TableRow>
    </div>
