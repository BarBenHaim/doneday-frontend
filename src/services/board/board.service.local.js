...
async function addExistingBoard(board) {
    try {
        const existingBoards = (await storageService.query(STORAGE_KEY)) || []

        existingBoards.push(board)

        await storageService._save(STORAGE_KEY, existingBoards)

        return board
    } catch (err) {
        console.error('Cannot add board to storage', err)
        throw err
    }
}
...
function _createBoardDemo(boardTitle, boardLabel) {
    ...
    return {
        ...
        members: [],
        groups: [
            getEmptyGroup('Group Title', { backgroundColor: '#579bfc' }, [task1, task2, task3]),
            getEmptyGroup('Group Title', { backgroundColor: '#a25ddc' }, [task4, task5]),
        ],
        activities: [],
        cmpsOrder: ['checkbox', 'title', 'description', 'priority', 'dueDate', 'memberIds', 'status', 'files'],
    }
}
